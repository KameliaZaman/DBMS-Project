<html>
	<head>
		<title>Bill</title>
	</head>
	
	<body>
		<img src="MARVEL.jpg" style="width:100%; height:100%; position:absolute; top:0; left:0; z-index:-5000;">
		<form action=""   method="post"   style="border:2px solid #a91cb0;   background-color: #a4f9ef7F;   text-align:center;">		
			<table align="center"   style=" font-style: italic;   font-size:20pt;   font-weight:Bold; ">
				<tr>
					<td>Enter Bill No: </td>
					<td> <input name="Bill_No" type="value"> </td>
				</tr>
				<tr>
					<td>Enter Cashier No: </td>
					<td> <input name="Cashier_No" type="value"> </td>
				</tr>
				<tr>
					<td>Enter Order Details: </td>
					<td> <input name="Order_Details" type="text"> </td>
				</tr>
				<tr>
					<td> <input name="x" type="submit" value="SAVE">
					 <input name="y" type="submit" value="DISPLAY"> </td>
				</tr>

			</table>
		</form>

		<?php

			$con=mysqli_connect("localhost","root","","restaurant");

			if(isset($_REQUEST['x']))
			{
				$a=$_REQUEST['Bill_No'];
				$b=$_REQUEST['Cashier_No'];
				$c=$_REQUEST['Order_Details'];

				$q="insert into Bill values($a,$b,'$c')";
				$result=mysqli_query($con,$q);
			}

			if(isset($_REQUEST['y']))
			{
				$disp="select * from Bill";
				$result=mysqli_query($con,$disp);
				echo '<table border=10 align="center"   style="border:2px solid #a91cb0; background-color: #bebdf5; font-style: italic; font-size:18;">';
					echo '<tr><th>Bill_No</th><th>Cashier_No</th><th>Order_Details</th></tr>';

					while($row=mysqli_fetch_array($result))
					{
						echo'<tr>';
							echo '<td>'.$row['Bill_No'].'</td>';
							echo '<td>'.$row['Cashier_No'].'</td>';
							echo '<td>'.$row['Order_Details'].'</td>';
						echo'</tr>';

					}

				echo '</table>';
			}

			mysqli_close($con);
		?>

	</body>
</html>